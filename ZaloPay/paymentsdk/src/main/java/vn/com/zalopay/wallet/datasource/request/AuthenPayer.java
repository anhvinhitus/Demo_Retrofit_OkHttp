...
public class AuthenPayer extends BaseRequest<StatusResponse> {
    private AdapterBase mAdapter;
    private String mTransID, mAuthenType, mAuthenValue;

    public AuthenPayer(AdapterBase pAdapter, String pTransID, String pAuthenType, String pAuthenValue) {
        super();
        mAdapter = pAdapter;
    ...
    protected void doRequest() {
        try {
            if (mAdapter.checkNetworkingAndShowRequest()) {
                DataRepository.shareInstance().setDataSourceListener(getDataSourceListener()).pushData(new AuthenPayerImpl(), getDataParams());
                ;
            }
        } catch (Exception ex) {
            Log.e(this, ex);
