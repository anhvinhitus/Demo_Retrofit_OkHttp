package vn.com.zalopay.wallet.view.dialog;
...
import vn.com.zalopay.wallet.listener.OnCustomContentDialogEventListener;
import vn.com.zalopay.wallet.listener.ZPWOnDialogListener;
...
import vn.com.zalopay.wallet.view.dialog.SweetAlertDialog.OnSweetClickListener;
...
public class DialogManager {
    ...
    private synchronized static void showProcessDialog(Activity pActivity, final long pStartTime, final ZPWOnProgressDialogTimeoutListener pCallback, long pTimeoutLoading) {
        try {
            if (pActivity == null) {
                pActivity = GlobalData.getMerchantActivity();
            }

            if (pActivity == null) {
                Log.d("showProcessDialog", "activity is null !");
                return;
            }
            ;
                            ...
                            synchronized (new Object()) {
                                try {
                                    if (mProgressDialog != null && mProgressDialog.isShowing() && mLastShowProcessDialog == pStartTime) {
                                        ...
                                        if (activity.get() != null && timeoutLoading.get() != null) {
    ...
    public synchronized static void showProcessDialog(Activity pActivity, ZPWOnProgressDialogTimeoutListener pCallback) {
        showProcessDialog(pActivity, System.currentTimeMillis(), pCallback, Constants.PROGRESS_DIALOG_TIMEOUT);
    }
        ...
        try {
            if (mProgressDialog != null && mProgressDialog.isShowing()) {
                ...
                if (BasePaymentActivity.getCurrentActivity() instanceof PaymentChannelActivity
                        ...
                        && ((PaymentChannelActivity) BasePaymentActivity.getCurrentActivity()).getAdapter().getGuiProcessor() != null) {
                    if (((PaymentChannelActivity) BasePaymentActivity.getCurrentActivity()).getAdapter().getGuiProcessor().isNeedToShowKeyBoardWhenCloseProcessDialog())
                        ((PaymentChannelActivity) BasePaymentActivity.getCurrentActivity()).getAdapter().getGuiProcessor().showKeyBoardAndResizeButtonsIfNotSwitchChannel();
                }

                Log.d("showProcessDialog", "Dismissed a processing dialog");

            }
            Log.d("showProcessDialog", " Call Dismissed dialog==" + mProgressDialog);
        } catch (Exception e) {
            Log.e("showProcessDialog", e);
        }
    }
    ...
    public synchronized static void showSweetDialogOptionNotice(final Activity pActivity, final String pMessage, final String pOKButton, final String pCancelButton, final ZPWOnEventConfirmDialogListener callback) {
        ...
        try {
            if (pActivity == null || pActivity.isFinishing()) {
                Log.d("showSweetDialogOptionNotice", "pActivity == null || pActivity.isFinishing()");
                return;
            }
            ...
            if (mConfirmSweetDialog.isShowing()) {
                Log.d("showSweetDialogOptionNotice", "===there're a mConfirmSweetDialog is showing====");
                return;
            }
     ...
     * @param callback     call back
     */
    public synchronized static void showSweetDialogNoInternet(final Activity pActivity, final String pMessage, final String pRightButton, final String pLeftButton, final OnCustomContentDialogEventListener callback) {
        ...
        try {
            ...
            if (mSweetDialogNoInternet.isShowing()) {
                Log.d("showSweetDialogNoInternet", "===there're a dialog is showing");
                return;
            }
